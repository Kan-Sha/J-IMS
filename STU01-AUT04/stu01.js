document.addEventListener('DOMContentLoaded', function () {
  ...
  const danhSachLop = [
    { value: '6a', ten: '6A', max: 30, hienTai: 30 }, // full
    { value: '6b', ten: '6B', max: 30, hienTai: 25 }, // còn 5
    { value: '7a', ten: '7A', max: 28, hienTai: 28 }, // full
    { value: '7b', ten: '7B', max: 28, hienTai: 20 }, // còn 8
    { value: '8a', ten: '8A', max: 25, hienTai: 24 }, // còn 1
    { value: '8b', ten: '8B', max: 25, hienTai: 10 }, // còn 15
  ];

  const lopConCho = danhSachLop.filter(l => l.hienTai < l.max);
  ...
  if (lopConCho.length === 0) {
    ...
    lopConCho.forEach(l => {
      const conLai = l.max - l.hienTai;
      const opt = document.createElement('option');
      opt.value = l.value;
      opt.textContent = l.ten;
      inputLop.appendChild(opt);
    });
  }

  taoSelectHienThi(document.getElementById('khung-lop'), inputLop);
  function taoMaHocVien() {
    const now   = new Date();
    const nam   = String(now.getFullYear());                      
    const thang = String(now.getMonth() + 1).padStart(2, '0');   
    const stt   = String(demStt).padStart(4, '0');              
    return `JS-${nam}${thang}-${stt}`;
  }

  inputMaHV.value = taoMaHocVien();
  ...
  nutLuu.addEventListener('click', function () {
    ...
    if (danhSachHocSinhTonTai.includes(email.toLowerCase())) {
      hienPopup('Học sinh đã tồn tại!');
      return;
    }
    ...
    setTimeout(() => {
      ...
      danhSachHocSinhTonTai.push(email.toLowerCase());
      demStt++; 
      ...
      hienPopup('Thêm học sinh thành công!');
    }, 800);
  });
...
const nutHocSinhSidebar = document.querySelector('[title="Học sinh"]');
